...
const Posts = () => {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      setLoading(true);
      // Make an API call or load more data here
      setTimeout(() => {
  ...
  const handleScroll = () => {
    ...
    const windowBottom = documentHeight - windowHeight;
    const scrollTop = window.pageYOffset || document.documentElement.scrollTop;

    if (windowBottom - scrollTop < 100 && !loading) {
      // Load more items when user reaches near the bottom (adjust the threshold as needed)
      setLoading(true);
    }
  };
